...
namespace RestaurantReservation.Business.ServiceClass
{
    public class ReservationService
    {
        ...
        public async Task<OrderDomain> GetOrdersForReservation(int reservationId)
        {
            try
            {
                var reservations = await _reservationRepository.GetReservationByIdAsync(reservationId);

                return reservations.Order;
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Error occurred while fetching order for reservations  {ReservationId}", reservationId);
                throw;
            }
        }
        public async Task<List<MenuItemDomain>> GetMenuItemsForReservationAsync(int reservationId)
        {
            try
            {
                var reservations = await _reservationRepository.GetReservationByIdAsync(reservationId);

                return reservations.Order.Items.Select(i=>i.Item).ToList();
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Error occurred while fetching MenuItem for reservations  {ReservationId}", reservationId);
                throw;
            }
        }
    }
